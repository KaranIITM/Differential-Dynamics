...
with ode_tab:
    ...
    def rkf45(f, t0, y0, t_end, h, tol=1e-6):
        ...
        while t < t_end:
            ...
            k6 = f(t + h/2,          y + h*(-8*k1/27 +2*k2 -3544*k3/2565
    ...
    def adams_bashforth4(f, t0, y0, h, n):
        ...
        for i in range(3, n):
            t = T[-1] + h
            y = Y[-1] + h/24*(55*f(T[-1], Y[-1]) -59*f(T[-2], Y[-2])
    ...
    def adams_moulton4(f, t0, y0, h, n):
        ...
        for i in range(3, n):
            t_pred = T[-1] + h
            y_pred = Y[-1] + h/24*(55*f(T[-1], Y[-1]) -59*f(T[-2], Y[-2])
                                   +37*f(T[-3], Y[-3]) -9*f(T[-4], Y[-4]))
            y_corr = Y[-1] + h/24*(9*f(t_pred, y_pred) +19*f(T[-1], Y[-1])
    ...
    if run_pde:
        ...
        if pde_type == "Heat Equation" and scheme in {"Forward-Euler", "Crank-Nicolson", "Backward-Euler"}:
            ...
            for _ in range(int(Nt)):
                u[0] = eval(bc_left, {"t": _*dt})
                u[-1] = eval(bc_right, {"t": _*dt})
